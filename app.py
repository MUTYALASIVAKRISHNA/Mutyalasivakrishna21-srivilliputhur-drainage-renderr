...
def ensure_environment():
    ...
    if not DB_PATH.exists():
        try:
            ...
            database.init_db()
            database.seed_data()
            print("Auto-initialized and seeded SQLite database on startup.")
        except Exception as e:
...
def get_ml_assets():
    ...
    if not metrics_path.exists():
        try:
            ...
            print(f"ML Pipeline auto-train notice: {e}")
    ...
    with open(str(metrics_path), 'r') as f:
        metrics = json.load(f)
    try:
        rf_causal = joblib.load(str(MODELS_DIR / 'rf_causal.joblib'))
        lr_causal = joblib.load(str(MODELS_DIR / 'logistic_causal.joblib'))
        rf_leakage = joblib.load(str(MODELS_DIR / 'rf_leakage.joblib'))
        lr_leakage = joblib.load(str(MODELS_DIR / 'logistic_leakage.joblib'))
    except Exception as e:
...
def api_predict():
    ...
    pipeline_type = data.get('pipeline_type', 'causal')
