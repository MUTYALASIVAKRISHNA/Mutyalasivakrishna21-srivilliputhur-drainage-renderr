...
class TestSrivilliputhurPrototype(unittest.TestCase):
    ...
    def test_all_web_routes(self):
        ...
        for r in routes:
            ...
            # Verify the small black PROTOTYPE DEMO badge is not present
            self.assertNotIn(b"PROTOTYPE DEMO", res.data, f"Route {r} should not have PROTOTYPE DEMO badge")

    # 3. Static Assets Verification
    def test_static_assets(self):
        # Hero scenic background
        res_img = self.client.get('/static/img/hero-bg.jpg')
        self.assertEqual(res_img.status_code, 200, "hero-bg.jpg must be accessible")
        self.assertGreater(len(res_img.data), 100000, "hero-bg.jpg must be valid image file")

        # Custom CSS
        res_css = self.client.get('/static/css/custom.css')
        self.assertEqual(res_css.status_code, 200, "custom.css must be accessible")

        # Main JS scripts
        for js in ['main.js', 'map.js', 'charts.js', 'prediction.js']:
            res_js = self.client.get(f'/static/js/{js}')
            self.assertEqual(res_js.status_code, 200, f"{js} must be accessible")

    # 4. REST API & ML Inference Tests
