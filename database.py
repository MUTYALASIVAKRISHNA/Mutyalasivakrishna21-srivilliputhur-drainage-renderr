...
def seed_data():
    ...
    layer1_path = RAW_DIR / 'Srivilliputhur_Complete_Dataset_Layer1_Layer2.xlsx'
    survey_path = RAW_DIR / 'Srivilliputhur_Drainage_Field_Survey_Template (1).xlsx'
    synthetic_path = RAW_DIR / 'Srivilliputhur_Synthetic_ML_Dataset.xlsx'

    if not layer1_path.exists():
        print(f"Notice: Source Excel {layer1_path} not found; skipping fresh import.")
        conn.close()
        return

    # 1. Seed Wards (1D_Ward_Data)
    cursor.execute("DELETE FROM wards")
    df_wards = pd.read_excel(str(layer1_path), sheet_name='1D_Ward_Data')
    ...
    df_stats = pd.read_excel(str(layer1_path), sheet_name='1C_Municipality_Stats')
    ...
    df_drains = pd.read_excel(str(layer1_path), sheet_name='1B_Verified_Drainage')
    ...
    df_syn = pd.read_excel(str(synthetic_path), sheet_name='Synthetic_Drains')
    ...
    df_inc = pd.read_excel(str(layer1_path), sheet_name='1A_Historical_Incidents')
    ...
    df_rain = pd.read_excel(str(layer1_path), sheet_name='1E_Rainfall_Portals')
    ...
    df_plas = pd.read_excel(str(layer1_path), sheet_name='1F_Plastic_Info')
    ...
    df_survey = pd.read_excel(str(survey_path), sheet_name='Drain_Survey')
